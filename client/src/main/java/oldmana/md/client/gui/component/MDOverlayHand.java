...
public class MDOverlayHand extends MDComponent
{
	...
	public MDOverlayHand(Card card)
	{
		...
		boolean canPlayJSN = card instanceof CardActionActionCounter && state != null && ((state.isTarget(player) && 
		...
		boolean canPlayRentCounter = card instanceof CardActionRentCounter && state != null && state instanceof ActionStateRent && 
				state.isTarget(player) && !state.isRefused(player) && !state.isAccepted(player);
		boolean canDiscard = state instanceof ActionStateDiscard;
